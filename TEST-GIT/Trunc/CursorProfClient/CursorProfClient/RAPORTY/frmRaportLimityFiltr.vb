Public Class frmRaportLimityFiltr
    Public frm As New frmRaportLimity
...
Dim ws As New wsCursorProf.CursorService
        ...
        Dim wsWynik As wsCursorProf.RaportLimityWczytajWynik

        'odczyt z serwera
        Try
            Cursor = Cursors.WaitCursor
            Application.DoEvents()
            wsWynik = ws.RaportLimityWczytaj(frmGlowna.sesja, Date.Parse(dtpDataOd.Value.ToString("yyyy-MM-dd")), Date.Parse(dtpDataDo.Value.ToString("yyyy-MM-dd")))
        ...
        If wsWynik.dane.Tables.Count > 0 Then
            If wsWynik.dane.Tables(0).Rows.Count < 1 Then
                MsgBox("Brak danych o limitach z wybranego okresu.", MsgBoxStyle.Exclamation, Me.Text)
                Return False
            End If
        Else
            MsgBox("Błąd wewnętrzny systemu. Serwer nie przesłał tabeli z raportem limitów." & frmGlowna.kontaktIt, MsgBoxStyle.Critical, Me.Text)
            Return False
            Exit Function
        End If
        frm.dtLimity = wsWynik.dane.Tables(0)
        frm.dgv.DataSource = frm.dtLimity
    ...
    Private Sub btnGenerujRaport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerujRaport.Click
        ...
        If Not wczytaj() Then
            Exit Sub
        Else
            frm.Text = "Raport limitów za okres od " & dtpDataOd.Text & " do " & dtpDataDo.Text
            frm.MdiParent = frmGlowna
            frm.Show()
            frm = New frmRaportLimity
        End If
