...
jQuery.fn.dataTableExt.oApi.fnGetHiddenTrNodes = function ( settings )
{
	var nodes;
...
jQuery.fn.dataTableExt.oApi.fnGetHiddenTdNodes = function (settings) 
    {
        var nodes;
        var display = jQuery('tbody td', settings.nTable);

            // DataTables 1.10
            var api = new jQuery.fn.dataTable.Api( settings );
            nodes = api.cells().nodes().toArray();
